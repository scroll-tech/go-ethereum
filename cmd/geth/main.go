...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.ScrollStagingFlag.Name):
		log.Info("Starting l2geth on Scroll Staging testnet...")
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.GlobalIsSet(utils.RopstenFlag.Name) && !ctx.GlobalIsSet(utils.RinkebyFlag.Name) && !ctx.GlobalIsSet(utils.GoerliFlag.Name) && !ctx.GlobalIsSet(utils.ScrollAlphaFlag.Name) && !ctx.GlobalIsSet(utils.ScrollStagingFlag.Name) && !ctx.GlobalIsSet(utils.DeveloperFlag.Name) {
