...
func (it *NodeIterator) step() error {
	...
	if !bytes.Equal(account.PoseidonCodeHash, emptyPoseidonCodeHash) {
		it.codeHash = common.BytesToHash(account.PoseidonCodeHash)
		addrHash := common.BytesToHash(it.stateIt.LeafKey())
		it.code, err = it.state.db.ContractCode(addrHash, common.BytesToHash(account.PoseidonCodeHash))
		if err != nil {
			return fmt.Errorf("code %x: %v", account.PoseidonCodeHash, err)
		}
	}
