...
func testGenerateWithManyExtraAccounts(t *testing.T, scheme string) {
	...
	}
	{
		...
		for i := 0; i < 1000; i++ {
			acc := &types.StateAccount{Balance: big.NewInt(int64(i)), Root: types.EmptyRootHash, KeccakCodeHash: types.EmptyKeccakCodeHash.Bytes(), PoseidonCodeHash: types.EmptyPoseidonCodeHash.Bytes()}
			val, _ := rlp.EncodeToBytes(acc)
...
func testGenerateWithExtraBeforeAndAfter(t *testing.T, scheme string) {
	...
	helper := newHelper(scheme)
	{
		acc := &types.StateAccount{Balance: big.NewInt(1), Root: types.EmptyRootHash, KeccakCodeHash: types.EmptyKeccakCodeHash.Bytes(), PoseidonCodeHash: types.EmptyPoseidonCodeHash.Bytes()}
		val, _ := rlp.EncodeToBytes(acc)
