...
// eciesDecrypt implements the ECIES decryption precompiled contract.
// It decrypts the input ciphertext using the sequencer's private key
// and returns the plaintext. The input is expected to be a valid ECIES
// ciphertext that was encrypted with the sequencer's public key.
...
var (
	errEciesDecryptInvalidCiphertextLength = errors.New("invalid ciphertext length")
	errEciesDecryptUnknownSequencerKey     = errors.New("internal error: unknown sequencer key")
	errEciesDecryptDecryptionFailed        = errors.New("decryption failed, invalid input or key")
	errEciesDecryptInvalidPlaintextLength  = errors.New("invalid plaintext length")
)

// RequiredGas returns the gas required to execute the precompiled contract
func (c *eciesDecrypt) RequiredGas(input []byte) uint64 {
	return 0
}

// Run executes the precompiled contract with given ciphertext parameter, returning the output and the used gas
func (c *eciesDecrypt) Run(ciphertext []byte) ([]byte, error) {
	...
	if len(plaintext) > eciesDecryptMaxPlaintextLength {
		return nil, errEciesDecryptInvalidPlaintextLength
	}

	// TODO: consider checking if the plaintext is valid UTF-8
	// if !utf8.Valid(plaintext) {
	// 	return "", ErrEciesPlaintextNotUTF8
	// }
...
func (c *eciesDecrypt) checkKeys(state stateDB) error {
	// check current key
	if local, commitment := crypto.Keccak256(c.currentPrivkey.PublicKey.Bytes(true)), validium.CurrentPubkey(state); !bytes.Equal(local, commitment) {
		return errEciesDecryptUnknownSequencerKey
	}

	// check prev key
	if local, commitment := crypto.Keccak256(c.prevPrivkey.PublicKey.Bytes(true)), validium.PrevPubkey(state); !bytes.Equal(local, commitment) {
		return errEciesDecryptUnknownSequencerKey
	}
...
func (c *eciesDecrypt) decrypt(ciphertext []byte) ([]byte, error) {
	// Attempt to decrypt the ciphertext using the current private key.
	if plaintext, err := validium.DecryptEcies(ciphertext, c.currentPrivkey); err == nil {
		return plaintext, nil
	}

	// Attempt to decrypt the ciphertext using the previous private key.
	if plaintext, err := validium.DecryptEcies(ciphertext, c.prevPrivkey); err == nil {
		return plaintext, nil
	}

	return nil, errEciesDecryptDecryptionFailed
}
