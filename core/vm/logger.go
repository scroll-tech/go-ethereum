...
func (s *StructLog) tryNewExtraData() *types.ExtraData {
	if s.ExtraData == nil {
...
func (l *StructLogger) CaptureState(pc uint64, op OpCode, gas, cost uint64, scope *ScopeContext, rData []byte, depth int, err error) {
	...
	if recordStorageDetail {
		...
		if err := traceStorageProof(l, scope, structlog.tryNewExtraData()); err != nil {
	...
	if ok {
		...
		for _, exec := range execFuncList {
			if err = exec(l, scope, structlog.tryNewExtraData()); err != nil {
				log.Error("Failed to trace data", "opcode", op.String(), "err", err)
			}
		}
	}
