...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")), func(t *testing.T) {
			...
			l1DataFee, err := fees.CalculateRollupFee(tx, statedb, test.Genesis.Config, context.BlockNumber, context.Time.Uint64())
			if err != nil {
...
func TestZeroValueToNotExitCall(t *testing.T) {
	...
	l1DataFee, err := fees.CalculateRollupFee(tx, statedb, params.MainnetChainConfig, context.BlockNumber, context.Time.Uint64())
	if err != nil {
		t.Fatalf("failed to calculate l1DataFee: %v", err)
	}
	st := core.NewStateTransition(evm, msg, new(core.GasPool).AddGas(tx.Gas()), l1DataFee)
