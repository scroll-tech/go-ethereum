	...
	for {
		...
		case errors.Is(err, circuitcapacitychecker.ErrBlockRowConsumptionOverflow):
			if w.current.tcount >= 1 {
				...
				log.Info("Skipping message", "tx", tx.Hash().String(), "block", w.current.header.Number, "reason", "block row consumption overflow")
				...
				if tx.IsL1MessageTx() {
					...
					queueIndex := tx.AsL1MessageTx().QueueIndex
					log.Info("Skipping L1 message", "queueIndex", queueIndex, "tx", tx.Hash().String(), "block", w.current.header.Number, "reason", "tx row consumption overflow")
					w.current.nextL1MsgIndex = queueIndex + 1
				} else {
					// Skip L2 transaction and all other transactions from the same sender account
					log.Info("Skipping L2 message", "tx", tx.Hash().String(), "block", w.current.header.Number, "reason", "tx row consumption overflow")
					txs.Pop()
				}
