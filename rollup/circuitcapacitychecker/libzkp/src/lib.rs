...
pub mod checker {
    ...
    pub unsafe extern "C" fn apply_tx(id: u64, tx_traces: *const c_char) -> *const c_char {
        let result = panic::catch_unwind(|| {
            log::debug!("ccc apply_tx raw input, id: {:?}, tx_traces: {:?}", id, utils::c_char_to_str(tx_traces));
    ...
    pub unsafe extern "C" fn apply_block(id: u64, block_trace: *const c_char) -> *const c_char {
        let result = panic::catch_unwind(|| {
            log::debug!("ccc apply_block raw input, id: {:?}, block_trace: {:?}", id, utils::c_char_to_str(block_trace));
