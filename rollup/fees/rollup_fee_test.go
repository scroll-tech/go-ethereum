...
func TestEstimateTxCompressionRatio(t *testing.T) {
	...
	t.Run("scr-transfer", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.NotNil(t, ratio)
		assert.Equal(t, big.NewInt(1_387_755_102), ratio) // 1.1x
	})
	...
	t.Run("syncswap-swap", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.NotNil(t, ratio)
		assert.Equal(t, big.NewInt(4_857_142_857), ratio) // 3.1x
	})
	...
	t.Run("uniswap-swap", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.NotNil(t, ratio)
		assert.Equal(t, big.NewInt(2_620_689_655), ratio) // 1.7x
	})
	...
	t.Run("etherfi-deposit", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.NotNil(t, ratio)
		assert.Equal(t, big.NewInt(1_788_944_723), ratio) // 1.4x
	})
	...
	t.Run("edgepushoracle-postupdate", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.NotNil(t, ratio)
		assert.Equal(t, big.NewInt(2_441_805_225), ratio) // 2.1x
	})
}
...
func TestSmallTxSizeThreshold(t *testing.T) {
	testCases := []struct {
		...
		{"all zeros", func(size int) []byte {
			data := make([]byte, size)
			// First 4 bytes are random
			for i := 0; i < 4 && i < size; i++ {
				data[i] = byte(rand.Intn(256))
			}
			// Rest are zeros (already initialized to 0)
			return data
		}},
