...
func (env *TraceEnv) GetBlockTrace(block *types.Block) (*types.BlockTrace, error) {
	...
	common.WithTimer(feedTxToTracerTimer, func() {
		for i, tx := range txs {
			...
			l1DataFee, err := fees.CalculateRollupFee(tx, env.state, env.chainConfig, block.Number(), block.Time())
			if err != nil {
				failed = err
				break
			}
...
func (env *TraceEnv) getTxResult(state *state.StateDB, index int, block *types.Block) error {
	...
	l1DataFee, err := fees.CalculateRollupFee(tx, state, env.chainConfig, block.Number(), block.Time())
	if err != nil {
		return err
	}
