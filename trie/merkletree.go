...
func NewMerkleTree(storage db.Storage, maxLevels int) (*MerkleTree, error) {
	return NewMerkleTreeWithRoot(storage, &HashZero, maxLevels)
}

// NewMerkleTree loads a new Merkletree. If in the sotrage already exists one
// will open that one, if not, will create a new one.
func NewMerkleTreeWithRoot(storage db.Storage, root *Hash, maxLevels int) (*MerkleTree, error) {
...
func VerifyProofSMT(rootKey *Hash, proof *Proof, k, v *big.Int) bool {
